...
// ---- experiment suggestions (AI-proposed, never auto-applied) ----------------
//
// lib/ai.js generateExperimentSuggestions() only produces text — these rows
// are the holding area until a human admin approves one (which turns it into
// a real experiment via addExperiment) or dismisses it. Nothing here ever
// touches a live ad account or a client's site on its own.

function createSuggestions(clientId, suggestions) {
  const db = getDb();
  const insert = db.prepare(
    `INSERT INTO experiment_suggestions (client_id, name, channel, rationale, expected_impact)
     VALUES (?, ?, ?, ?, ?)`
  );
  const insertAll = db.transaction((rows) => {
    for (const s of rows) {
      insert.run(clientId, s.name, s.channel, s.rationale, s.expectedImpact);
    }
  });
  insertAll(suggestions);
  return listPendingSuggestions(clientId);
}

function listPendingSuggestions(clientId) {
  const db = getDb();
  return db
    .prepare(
      "SELECT * FROM experiment_suggestions WHERE client_id = ? AND status = 'pending' ORDER BY created_at DESC"
    )
    .all(clientId);
}
...
function getSuggestionById(id) {
  const db = getDb();
  return db.prepare("SELECT * FROM experiment_suggestions WHERE id = ?").get(id);
}

/** Turns an approved suggestion into a real, running experiment. */
function approveSuggestion(id) {
  const db = getDb();
  const suggestion = getSuggestionById(id);
  if (!suggestion || suggestion.status !== "pending") return null;

  const experiment = addExperiment(suggestion.client_id, {
  ...
  db.prepare("UPDATE experiment_suggestions SET status = 'approved' WHERE id = ?").run(id);
  return experiment;
}
...
function dismissSuggestion(id) {
  const db = getDb();
  const info = db
    .prepare("UPDATE experiment_suggestions SET status = 'dismissed' WHERE id = ? AND status = 'pending'")
    .run(id);
...
function nameFromEmail(email) {
  ...
  if (!label) return "New client";
  return label.charAt(0).toUpperCase() + label.slice(1);
}

/**
 * Automatic onboarding: a Whop payment came in for an email with no
 * existing client_user — provision a brand-new client, mark them active,
 * and create an invite so they can set a password and get straight into
 * the portal. Called from the webhook handler; never invoked for events
 * that aren't a fresh "active" signal (cancellations/past_due for an
 * unknown email have nothing to provision).
 */
function autoProvisionClientFromPayment({ email, name, membershipId }) {
  const client = createClient({ name: name || nameFromEmail(email) });
  setBillingStatus(client.id, "active", membershipId);
  const invite = createInvite(client.id, email);
...
module.exports = {
  ...
  autoProvisionClientFromPayment,
